...
from acl_miniconf.data import Paper, WORKSHOP
...
CUSTOM_PAPER_YML = {
    ...
    Path('workshop-data/Narrative-Understanding'): Path('workshop-data/Narrative-Understanding/output/inputs/'),
...
def main(workshop_data_dir: Path = Path('workshop-data'), output_dir: Path = Path('data/acl_2023/data')):
    ...
    for workshop_dir in track(list(workshop_data_dir.glob('*'))):
        ...
        if not (workshop_dir / 'conference_details.yml').exists():
            raise ValueError(f'Workshop conference_details.yml does not exist for: {workshop_dir}')
        ...
        for p in papers:
            ...
            workshop_papers.append(Paper(
                ...
                tldr=maybe_abstract[:TLDR_LENGTH] if isinstance(maybe_abstract, str) else "",
                event_ids=[],
                program=WORKSHOP,
            ).dict())
